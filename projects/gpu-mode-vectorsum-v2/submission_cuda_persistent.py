...
THREADS = 256
ITEMS_PER_THREAD = 32
ELEMENTS_PER_CHUNK = THREADS * ITEMS_PER_THREAD
GRID_BLOCKS = 512
...
constexpr int ELEMENTS_PER_CHUNK = THREADS * ITEMS_PER_THREAD;
...
__global__ void persistent_partial_sum_kernel(
    ...
    for (int64_t block_start = static_cast<int64_t>(blockIdx.x) * ELEMENTS_PER_CHUNK;
         block_start < n_elements;
         block_start += static_cast<int64_t>(gridDim.x) * ELEMENTS_PER_CHUNK) {{
        #pragma unroll
...
__global__ void final_sum_kernel(
    ...
    for (int64_t offset = threadIdx.x; offset < num_partials; offset += THREADS) {{
...
torch::Tensor vector_sum_stage1(torch::Tensor x, torch::Tensor partial, int64_t n_elements, int64_t num_partials) {{
    ...
    persistent_partial_sum_kernel<<<num_partials, THREADS>>>(
        x.data_ptr<float>(),
...
torch::Tensor vector_sum_stage2(torch::Tensor partial, torch::Tensor output, int64_t num_partials) {{
    TORCH_CHECK(partial.device().is_cuda(), "partial must be a CUDA tensor");
...
_CUDA_MODULE = load_inline(
    name="vectorsum_cuda_persistent_g512",
...
def _partial_buffer(x: torch.Tensor, num_partials: int) -> torch.Tensor:
    key = (_device_index(x), num_partials, torch.float32)
    partial = _partial_cache.get(key)
    if partial is None or partial.device != x.device:
        partial = torch.empty((num_partials,), device=x.device, dtype=torch.float32)
...
def custom_kernel(data: input_t) -> output_t:
    x, output = data
    n_elements = x.numel()
    num_partials = min(GRID_BLOCKS, triton.cdiv(n_elements, ELEMENTS_PER_CHUNK))
    partial = _partial_buffer(x, num_partials)

    _CUDA_MODULE.vector_sum_stage1(x, partial, n_elements, num_partials)
    _CUDA_MODULE.vector_sum_stage2(partial, output, num_partials)
    return output[0]
