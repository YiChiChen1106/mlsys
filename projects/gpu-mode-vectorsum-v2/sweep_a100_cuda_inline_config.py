...
class Candidate:
    ...
    def module_name(self) -> str:
        return f"vectorsum_cuda_inline_t{self.threads}_i{self.items_per_thread}"


DEFAULT_CANDIDATES = (
    # Keep this list small because Popcorn A100 submissions are rate-limited.
    ...
    Candidate(threads=512, items_per_thread=16, filename="submission_cuda_t512_i16.py"),
    Candidate(threads=512, items_per_thread=32, filename="submission_cuda_t512_i32.py"),
)
        ...
        for part in raw.split(","):
            ...
            candidates.append(
                Candidate(
                    ...
                    filename=f"submission_cuda_t{threads}_i{items_per_thread}.py",
                )
            )
...
def run_benchmark(candidate: Candidate, output_dir: Path) -> dict[str, object]:
    ...
    result_path = output_dir / f"benchmark-cuda-t{candidate.threads}-i{candidate.items_per_thread}-a100.json"
    command = [
        "popcorn-cli",
        "submit",
        "--no-tui",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate and optionally benchmark CUDA inline config variants.")
    parser.add_argument("--submission", default="submission_cuda_inline_v2.py", help="Base CUDA inline v2 file to mutate.")
    parser.add_argument(
        "--output-dir",
        default="outputs/cuda-inline-config-sweep",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Candidate specs like 128x32 or comma-separated groups like 128x32,256x64.",
    )
